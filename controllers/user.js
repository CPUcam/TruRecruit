...
exports.postSearch = (req, res) => {
  ...
  User.findOne( { code: req.body.code }, (err, user) => {
    if (!user) {
      req.flash('errors', { msg: 'User with code not found. '});
      res.redirect('/search');
    }
    else {
      var result = {
        name: user.email,
        code: user.code,
        path: "/viewer/#../uploads/" + user.code + ".pdf"
      };

      fs.open("uploads/" + user.code + ".pdf", "r", (err, file) => {
        if (file) {
           return res.render('account/search', result);
        }

        else {
          var params = {
            localFile: "uploads/" + user.code + ".pdf",
          ...
          res.render('account/search', result);
        }
      });
    }
  });
